...
export default function YoutubeAudioPlayer({
  ...
  // Fetch YouTube video title via noembed
  ...
  // Send command to file origin iframe player
  ...
  // Load track using file origin method
  ...
  // Load track using YouTube Iframe API
  ...
  const handlePlayPause = () => {
    if (!isUnlockedRef.current) return; // Wait until enterSite
    playCurrent();
  };
  ...
  // Load and play automatically once unlocked
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Handle iframe message events (file origin mode callbacks)
  ...
  // Update track title when track changes or lang changes
  useEffect(() => {
    fetchTitleForTrack(YT_PLAYLIST[trackIndex].id);
  }, [trackIndex, lang]);

  // Compute progress bar filling
